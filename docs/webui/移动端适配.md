...
| 底部生成统计条看不清 | 每项都被 `text-overflow: ellipsis` 截成 `1…`/`LL…`；有壁纸时又是低对比灰字 |
| 侧栏拉开后只能点那个小按钮关 | 遮罩是 `box-shadow` 铺的，**不接收点击** |
...
`.generation-stats-line` 是 `display:flex` + `white-space:nowrap`，
每一项又都带 `text-overflow: ellipsis`。桌面端一行放得下所以看不出来；
手机上一行放不下，5 项会被**一起压扁**成 `1…`、`LL…`、`缓…`、`输入 10 t…`
——不是字小，是每项都被截断了。窄屏改为换行（两行看全，好过一行全是省略号）。

第二层问题是颜色：开了聊天壁纸时，上游会把 `--chat-wallpaper-content-fg`
套到聊天标题、作者名、正文气泡上（`chat-background.css:72`），**唯独漏了这条统计栏**，
于是壁纸一亮，`--muted` 的灰色小字就糊进背景里。按上游同一套变量补上。
（这一条严格说桌面端也中招，此处仍限定在窄屏，不动桌面观感。）

### 8. 点抽屉外的空白处关闭侧栏

上游的抽屉遮罩是 `box-shadow: 0 0 0 100vmax ...` 铺出来的——
它只是**画**出一层暗色，**不接收点击**。所以照着直觉点旁边什么也不会发生，
只能去够侧栏头部那个小按钮，不符合手机习惯。

注入脚本补上这条：抽屉展开时点到 `.side-panel-shell` 以外的地方就收起。
三个例外：抽屉内部、底部图标栏（切分区）、标题栏的展开按钮（它自己会处理，
否则会被这条规则当场关掉）。
`collapseDrawer` 也加了同拍去重——"点会话"和"点空白"两条规则可能同时命中。
...
- **M-stats**：统计条各项完整显示（无省略号），且开壁纸时已换成壁纸前景色
- **M-outside**：侧栏展开时点抽屉外的空白处，侧栏收起
...
**三、探针里不要用固定等待。**`tap` 流程原本固定等 800ms 再写报告，
偶发地会在会话还没渲染完时就下结论，导致 M-text / M-wallpaper / M-stats
一起误报（重跑又全绿）。改成轮询到"消息出现**且**统计条挂上"为止
（统计条要等整轮生成结束才渲染，只看消息会在生成中途就退出），
并把实际等待时长一并上报，方便判断余量。
