...
**前三位 = 对应的上游版本，第四位 = 我们在这个上游版本上的第几次发布。**

| tag | 镜像 tag | 含义 |
| --- | --- | --- |
| `webui-v0.1.2` | `0.1.2` | 首次跟进上游 0.1.2 |
| `webui-v0.1.2.1` | `0.1.2.1` | 在 0.1.2 基础上的第 1 次修补 |
| `webui-v0.1.3` | `0.1.3` | 上游发了 0.1.3，重新对齐 |

为什么不直接 `0.1.3`：那会让人以为上游发了 0.1.3。四段读作"上游 0.1.2 + 我们的
第 1 次修补"，信息不丢。

**已知取舍**：四段版本不是严格 SemVer（SemVer 只允许三段）。本项目里没有东西解析它——
`package.json` 的 `version` 门禁禁止改动、Docker tag 与 git tag 都是自由格式。
只有当以后接入会解析镜像 tag 做排序的自动更新工具（Renovate / Watchtower 之类）时才需要重新考虑。
